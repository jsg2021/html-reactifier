...
const htmlEntitySwizzler = (entityTag, entity) =>
	(
		//Trust the case at first,
		(entity = HTML_ENTITIES[entity]
			//then try a lowercased,
			|| HTML_ENTITIES[entity.toLowerCase()]
			//finally fallback with what they gave us.
			|| entity
		),
		//return:
		`&#${entity};`
...
const swizzleEntities = (html) => html.replace(ENTITY_MATCHER, htmlEntitySwizzler);
...
const getMatcher = tag =>
	getMatcher[tag] || (getMatcher[tag] = new RegExp(`(<${tag})([^>]*)(>)`, 'igm'));

const unaryFix = (o, tag, attrs, closer) =>
	(attrs.length === 0 || attrs.charAt(attrs.length - 1) !== '/')
		? (tag + attrs + '/' + closer)
		: o;

const ensureUnaryTagsAreClosedXML = (html) =>
	UNARY_ELEMENTS.reduce((output, tagName) => output.replace(getMatcher(tagName), unaryFix), html);

const crudeHTMLToXML = (html) =>
	ensureUnaryTagsAreClosedXML(swizzleEntities(html));
...
function processNode (n, isWidget) {
	const renderFunction = (isWidget && isWidget(n.tag, n.attrib, n)) ? 'renderWidget' : void 0;

	const el = reactifyElement(
		getLiteral(n.tag),
		n.attrib,
...
function processChildren (n, isWidget) {
	const text = n.text && getLiteral(n.text);
	const c = n.getchildren().map(x => processNode(x, isWidget));
...
export default function (html, isWidgetCallback) {
	try {
		const xml = crudeHTMLToXML(html);

		const root = et.parse(`<root>${xml}</root>`).getroot();
		const children = processChildren(root, isWidgetCallback);
